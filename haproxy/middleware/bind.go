...
func (c *Client) GetBind(bindName string, parentName string) (*models.Bind, error) {
	url := fmt.Sprintf("%s/services/haproxy/configuration/binds/%s?parent_type=frontend&parent_name=%s", c.base_url, bindName, parentName)
	req, err := http.NewRequest("GET", url, nil)
	if err != nil {
		return nil, err
	}
...
func (c *Client) CreateBind(transactionId string, bind models.Bind, parentName string) (*models.Bind, error) {
	url := fmt.Sprintf("%s/services/haproxy/configuration/binds/?parent_type=frontend&parent_name=%s&transaction_id=%s", c.base_url, parentName, transactionId)
...
func (c *Client) UpdateBind(transactionId string, bind models.Bind, parentName string) (*models.Bind, error) {
	url := fmt.Sprintf("%s/services/haproxy/configuration/binds/%s?transaction_id=%s&parent_type=frontend&parent_name=%s", c.base_url, bind.Name, transactionId, parentName)
	bodyStr, _ := json.Marshal(bind)
...
func (c *Client) DeleteBind(transactionId string, bindName string, parentName string) error {
	url := fmt.Sprintf("%s/services/haproxy/configuration/binds/%s?transaction_id=%s&parent_type=frontend&parent_name=%s", c.base_url, bindName, transactionId, parentName)
