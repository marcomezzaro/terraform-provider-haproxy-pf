...
func (c *Client) GetServer(serverName string, parentName string) (*models.Server, error) {
	url := fmt.Sprintf("%s/services/haproxy/configuration/servers/%s?parent_type=backend&parent_name=%s", c.base_url, serverName, parentName)
	req, err := http.NewRequest("GET", url, nil)
	if err != nil {
		return nil, err
	}
...
func (c *Client) CreateServer(transactionId string, server models.Server, parentName string) (*models.Server, error) {
	url := fmt.Sprintf("%s/services/haproxy/configuration/servers/?parent_type=backend&parent_name=%s&transaction_id=%s", c.base_url, parentName, transactionId)
...
func (c *Client) UpdateServer(transactionId string, server models.Server, parentName string) (*models.Server, error) {
	url := fmt.Sprintf("%s/services/haproxy/configuration/servers/%s?transaction_id=%s&parent_type=backend&parent_name=%s", c.base_url, server.Name, transactionId, parentName)
	bodyStr, _ := json.Marshal(server)
...
func (c *Client) DeleteServer(transactionId string, serverName string, parentName string) error {
	url := fmt.Sprintf("%s/services/haproxy/configuration/servers/%s?transaction_id=%s&parent_type=backend&parent_name=%s", c.base_url, serverName, transactionId, parentName)
